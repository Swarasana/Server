...
export class ExhibitionRepository {
  static async findAll(
    ...
    if (params.q) {
      ...
      query = query.or(
        `name.ilike.${searchTerm},description.ilike.${searchTerm},curator_name.ilike.${searchTerm},location.ilike.${searchTerm}`
      );
    }
  ...
  static async findCollectionsByExhibitionId(
    ...
    if (params.q) {
      const searchTerm = `%${params.q}%`;
      query = query.or(
        `name.ilike.${searchTerm},artist_name.ilike.${searchTerm}`
      );
    }
  ...
  static async addCollection(row: any) {
    const { data, error } = await supabase
      .from("exhibition_collections")
      .insert(row)
      .select()
  ...
  static async addCollectionsBulk(rows: any[]) {
    const { data, error } = await supabase
      .from("exhibition_collections")
      .insert(rows)
      .select();
